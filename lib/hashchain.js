 ...
 * against the previous block and signing the resulting hash.
 *
 * When `signingKeyPem` is supplied (the normal path now — see the /issue and
 * /revoke routes, which require the operator to submit the institution's
 * private key), that exact key material is used to sign, not a server-cached
 * copy. Falling back to the institution's stored key only happens when no
 * key is supplied at all (used by lib/seedData.js to bootstrap the two demo
 * credentials on first boot, before anyone has logged in).
 */
export function appendBlock(store, { type, payload, institutionId = null, signingKeyPem = null }) {
  ...
  let signature = null;
  if (signingKeyPem) {
    signature = signHex(hash, signingKeyPem);
